...
class RaftNode:
    def __init__(self, node_id, peers, config):
        self.node_id = str(node_id)
        self.peers = [str(p) for p in peers]
        self.config = config
        
        # Timing parameters
        self.election_timeout_min = config.get("raft", {}).get("election_timeout_min", 0.150)
        self.election_timeout_max = config.get("raft", {}).get("election_timeout_max", 0.300)
        self.heartbeat_interval = config.get("raft", {}).get("heartbeat_interval", 0.050)
    ...
    def load_persistent_state(self, state_data, log_data):
        ...
        self.log.last_snapshot_index = log_data.get("last_snapshot_index", 0)
        self.log.last_snapshot_term = log_data.get("last_snapshot_term", 0)
        self.log.entries = log_data.get("entries", [])
    ...
    def get_persistent_state_dict(self):
        return {
    ...
    def tick(self):
        """
        Increments election timer. Returns election vote message if candidate triggers.
        """
    ...
    def handle_rpc_response(self, peer_id, request_msg, response_msg):
        if not response_msg:
            return
        ...
        if self.state == "candidate" and req_type == "RequestVote":
            ...
            if response_msg.get("success"):
                ...
                self.next_index[str(peer_id)] = match_idx + 1
                self.match_index[str(peer_id)] = match_idx
                self.update_leader_commit_index()
            else:
                self.next_index[str(peer_id)] = max(1, self.next_index[str(peer_id)] - 1)

        elif self.state == "leader" and req_type == "InstallSnapshot":
            if response_msg.get("success"):
                self.next_index[str(peer_id)] = self.log.last_snapshot_index + 1
                self.match_index[str(peer_id)] = self.log.last_snapshot_index
    ...
    def get_peer_replication_payload(self, peer_id):
        peer_id = str(peer_id)
